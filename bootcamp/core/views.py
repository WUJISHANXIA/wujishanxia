...
def save_markdown_editor_picture(request):
    md5 = hashlib.md5()
    try:
        mk_pictures = django_settings.MEDIA_ROOT + '/markdown_pictures/'
        if not os.path.exists(mk_pictures):
            os.makedirs(mk_pictures)
        f = request.FILES['editormd-image-file']
        md5.update(f.read())
        file_md5 = md5.hexdigest()
        filename = django_settings.MEDIA_ROOT + '/markdown_pictures/' + file_md5 + '.jpg'
        if os.path.exists(filename):
            return JsonResponse({'success': 1, 'message': 'upload pic success',
                                 'url': '/media/markdown_pictures/' + file_md5 + '.jpg'})
        with open(filename, 'wb+') as destination:
            for chunk in f.chunks():
                destination.write(chunk)
        return JsonResponse({'success': 1, 'message': 'upload pic success',
